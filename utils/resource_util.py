...
Utils for managing resources like cluster, resource pool and host
'''
...
def get_cluster_mor_for_vm(session, vm_uuid):
    if cache.VCCache.get_cluster_mor_for_vm(vm_uuid):
        return cache.VCCache.get_cluster_mor_for_vm(vm_uuid)
    vm_mor = get_vm_mor_for_uuid(session, vm_uuid)
    if vm_mor:
        host_mor = session._call_method(
            vim_util, "get_dynamic_property", vm_mor,
