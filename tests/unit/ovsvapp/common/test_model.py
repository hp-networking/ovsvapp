...
class CommonModelTestCase(test.TestCase):
    ...
    def test_model_virtualnic(self):
        ...
        nic_type = "VMXNET"
        virtual_nic = model.\
            VirtualNic(mac_address, port_uuid,
    ...
    def test_model_ipconfig(self):
        ip_address = "100.100.100.100"
        subnet_mask = "255.255.255.0"
        gateway = "100.100.100.1"
        ipconfig = model.IPConfig(ip_address, subnet_mask, gateway)
        self.assertTrue(ipconfig.uuid is not None, "uuid is none")
        self.assertTrue(ipconfig.ip_address == ip_address,
                        "ip_address does not match")
        self.assertTrue(ipconfig.subnet_mask == subnet_mask,
                        "subnet_mask does not match")
        self.assertTrue(ipconfig.gateway == gateway,
                        "gateway does not match")
        self.assertTrue(ipconfig.dhcp_enabled is True,
                        "dhcp_enabled does not match")
        self.assertTrue(ipconfig.address_type == model.IPAddressType.IPV4,
                        "address_type does not match")
